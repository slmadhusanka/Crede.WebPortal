...
{
    public partial class ListOfErrorCategory : System.Web.UI.Page
    {
        ...
        private void LoadGrid()
        {
            try
            {
                List<ErrorCategory> LIST_OBJ_Regions = new List<ErrorCategory>();
                LIST_OBJ_Regions = ClsSystem.GetAll_Error_Category();
                //DataView dv = new DataView();
                if (LIST_OBJ_Regions.Count != 0)
                {
                    BUSessionUtility.BUSessionContainer.OBJ_CLASS1 = LIST_OBJ_Regions;
                    gvErrorCat.DataSource = LIST_OBJ_Regions;
                    gvErrorCat.DataBind();
                }
                else
                {
                    gvErrorCat.DataSource = string.Empty.ToList();
                    gvErrorCat.DataBind();
                }

                MakeAccessible(gvErrorCat);
            }
            catch (Exception ex)
            {
        ...
        {
            Response.Redirect("~/SystemPages/AddErrorCategory.aspx");
        }
        ...
        public static bool DeleteERROR(string id)
        {
            ...
            try
            {
                returnVal = ClsSystem.DeleteErrorCategory(id);
            }
            catch (Exception ex)
            {
                returnVal = false;
        ...
        protected void gvRegion_RowEditing(object sender, System.Web.UI.WebControls.GridViewEditEventArgs e)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(gvErrorCat.Rows[e.NewEditIndex].Cells[0].Text))
                {
                    ErrorCategory objRegion = new ErrorCategory();
                    objRegion.ID = gvErrorCat.Rows[e.NewEditIndex].Cells[0].Text;
                    objRegion.Description = gvErrorCat.Rows[e.NewEditIndex].Cells[1].Text.Replace("&#39;", "'").Replace("&amp;", "&").Replace("&quot;", "\"").Replace("&lt;", "<").Replace("&gt;", ">");
                    if (gvErrorCat.Rows[e.NewEditIndex].Cells[3].Text == "Yes")
                    ...
                    Response.Redirect("~/SystemPages/UpdateErrorCategory.aspx");
                }
            }
            catch (System.Threading.ThreadAbortException)
            {
