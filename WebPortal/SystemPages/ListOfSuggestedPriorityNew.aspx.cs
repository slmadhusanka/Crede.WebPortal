...
namespace WebPortal.SystemPages
{
    ...
    public partial class ListOfSuggestedPriorityNew : System.Web.UI.Page
    {
        ...
        private void LoadGrid()
        {
            try
            {
                List<SuggestedPriority> LIST_OBJ_Regions = new List<SuggestedPriority>();
                LIST_OBJ_Regions = ClsSystem.GetAllSuggestedPriorityTypeList();
                //DataView dv = new DataView();
                if (LIST_OBJ_Regions.Count != 0)
                {
                    BUSessionUtility.BUSessionContainer.OBJ_CLASS1 = LIST_OBJ_Regions;
                    gvSuggestedPriority.DataSource = LIST_OBJ_Regions;
                    gvSuggestedPriority.DataBind();
                }
                else
                {
                    gvSuggestedPriority.DataSource = string.Empty.ToList();
                    gvSuggestedPriority.DataBind();
                }

                MakeAccessible(gvSuggestedPriority);
            }
            catch (Exception ex)
            {
        ...
        {
            Response.Redirect("~/SystemPages/AddSuggestedPriorityType.aspx");
        }
        ...
        public static bool DeleteERROR(string id)
        {
            ...
            try
            {
                returnVal = ClsSystem.DeleteSuggestedPriorityType(id);
            }
            catch (Exception ex)
            {
                returnVal = false;
        ...
        protected void gvRegion_RowEditing(object sender, System.Web.UI.WebControls.GridViewEditEventArgs e)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(gvSuggestedPriority.Rows[e.NewEditIndex].Cells[0].Text))
                {
                    SuggestedPriority objRegion = new SuggestedPriority();
                    objRegion.ID = gvSuggestedPriority.Rows[e.NewEditIndex].Cells[0].Text;
                    objRegion.Description = gvSuggestedPriority.Rows[e.NewEditIndex].Cells[1].Text.Replace("&#39;", "'").Replace("&amp;", "&").Replace("&quot;", "\"").Replace("&lt;", "<").Replace("&gt;", ">");
                    if (gvSuggestedPriority.Rows[e.NewEditIndex].Cells[3].Text == "Yes")
                    ...
                    Response.Redirect("~/SystemPages/UpdateSuggestedPriorityNew.aspx");
                }
            }
            catch (System.Threading.ThreadAbortException)
            {
